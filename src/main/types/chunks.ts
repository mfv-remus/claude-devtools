...
/**
 * Hook chunk - represents a CLI hook execution (SessionStart, UserPromptSubmit,
 * PostToolUse, etc.). Distinct from SystemChunk because hooks are generated by the
 * CLI/hook scripts, not by slash-command output or the LLM.
 */
export interface HookChunk extends BaseChunk {
  chunkType: 'hook';
  message: ParsedMessage;
}

/**
 * A chunk can be either a user input, AI response, system output, compact boundary,
 * or hook execution.
 * This discriminated union enables separate visualization and processing.
 */
export type Chunk = UserChunk | AIChunk | SystemChunk | CompactChunk | HookChunk;
...
/**
 * Enhanced hook chunk with additional metadata.
 */
export interface EnhancedHookChunk extends HookChunk {
  /** Raw messages for debug sidebar */
  rawMessages: ParsedMessage[];
}

/**
 * Enhanced chunk can be user, AI, system, compact, or hook type.
 */
...
/**
 * Type guard to check if a chunk is a HookChunk.
 */
export function isHookChunk(chunk: Chunk | EnhancedChunk): chunk is HookChunk {
  return 'chunkType' in chunk && chunk.chunkType === 'hook';
}
